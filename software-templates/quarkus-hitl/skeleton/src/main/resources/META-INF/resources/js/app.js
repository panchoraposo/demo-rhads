...
function displayValue(carValue) {
    if (!carValue) {
        return '';
    }
    const match = String(carValue).match(/\$[0-9,]+/);
    return match ? match[0] : String(carValue);
}
...
function createApprovalCard(proposal) {
    ...
    card.innerHTML = `
        <div class="approval-card-header">
            ...
            <div class="vehicle-value">${escapeHtml(displayValue(proposal.carValue))}</div>
        </div>
        
        <div class="approval-card-body">
            <div class="info-row">
                <span class="info-label">Car #${proposal.carNumber}</span>
                <span class="info-label">Condition: ${escapeHtml(proposal.carCondition || 'n/a')}</span>
            </div>
            
            <div class="damage-section">
                <div class="section-title">Damage Report</div>
                <div class="damage-text">${escapeHtml(proposal.rentalFeedback || proposal.carCondition || 'No feedback provided')}</div>
            </div>
            
            <div class="proposal-section">
                <div class="section-title">AI Recommendation</div>
                <div class="proposal-action">
                    <span class="action-badge">${escapeHtml(proposal.proposedDisposition || 'SCRAP')}</span>
                    <span class="action-reason">${escapeHtml(proposal.dispositionReason || '')}</span>
                </div>
            </div>
            ${proposal.carValue && String(proposal.carValue).length > 12 ? `
            <div class="proposal-section">
                <div class="section-title">PricingAgent</div>
                <div class="action-reason">${escapeHtml(proposal.carValue)}</div>
            </div>` : ''}
        </div>
